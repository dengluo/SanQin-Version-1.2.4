...
package com.pbids.sanqin.ui.view.curl;
...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
	...
	// Shows one page at the center of view_donate_records.
...
//		setZOrderMediaOverlay(true);
	...
	public void onDrawFrame() {
		...
		if(mChangeIndex!=mCurrentIndex){
			mChangeIndex = mCurrentIndex;
			onIndexChanageLisenear.indexChangeLisenear(mChangeIndex);
		}
//		mAnimate = false;
		Log.i("wzh","mCurrentIndex111: "+mCurrentIndex);
	}

	OnIndexChanageLisenear onIndexChanageLisenear;

	public void setOnIndexChanageLisenear(OnIndexChanageLisenear onIndexChanageLisenear){
		this.onIndexChanageLisenear = onIndexChanageLisenear;
	}

	public interface OnIndexChanageLisenear{
		void indexChangeLisenear(int index);
	}
...
//		}
		...
		case MotionEvent.ACTION_DOWN: {
			if(me.getX()>ScreenUtils.getScreenWidth()/2){
...
//			if(Math.abs(me.getX()-downX)<5 && Math.abs(me.getY()-downY)<5){
//				return false;
//			}
	...
	/**
	 * Sets view_donate_records mode. Value can be either SHOW_ONE_PAGE or SHOW_TWO_PAGES. In
	...
	private void startCurl(int page) {
		...
		case CURL_LEFT: {
			...
			// If there is something to show on right page insert it to renderer.
			...
			// How dragging previous page happens depends on view_donate_records mode.
