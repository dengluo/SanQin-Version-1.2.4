...
import com.pbids.sanqin.ui.adapter.SystemMessageAdapter;
import com.pbids.sanqin.ui.viewholder.SystemMessageViewHolder;
...
public class SystemMessageActivity extends UI implements TAdapterDelegate,
    ...
    public void onAgree(SystemMessage message) {
        if(message.getType()==SystemMessageType.TeamInvite){
            EventBus.getDefault().post(new ValJoinTeam());
    ...
    public void joinTeamEvent(TeamNumber teamNumber){
        if(queryMyTeamSize()<teamNumber.getNumber()){
            onSystemNotificationDeal(message, true);
        }else if(teamNumber.getNumber()==-1){
            onSystemNotificationDeal(message, true);
        } else{
            final NimEditTipsDialog nimEditTipsDialog = new NimEditTipsDialog(SystemMessageActivity.this);
            ...
            showDialog(SystemMessageActivity.this,nimEditTipsDialog,0.8, getResources().getDimensionPixelSize(R.dimen.dp_230));
            nimEditTipsDialog.setViewType(NimEditTipsDialog.CHOSES);
            nimEditTipsDialog.getCancle().setVisibility(View.GONE);
            nimEditTipsDialog.setDescTitle("您的入群数量已达到"+teamNumber.getNumber()+"个，如需加入新群需先退出已加入的群，方可加入新群！");
