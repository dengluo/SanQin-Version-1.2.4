...
public abstract class BaseReadView extends View {
    ...
    public BaseReadView(Context context, ArrayList<ViewGroup> views,
...
//        mScreenHeight = ScreenUtils.getScreenHeight();
        mScreenWidth = (int)context.getResources().getDimension(R.dimen.dp_346);
        mScreenHeight = (int)context.getResources().getDimension(R.dimen.dp_422);
        ...
        pagefactory = new PageFactory(getContext(), views);
...
//            isPrepared = true;
//        }
        pagefactory.onDraw(mCurrentPageCanvas,pagefactory.getCurrentPage());
        postInvalidate();
    }
...
//    }
    ...
    public OnSrollLisenear srollLisenear;

    public void setSrollLisenear(OnSrollLisenear srollLisenear){
        this.srollLisenear = srollLisenear;
    }

    public interface OnSrollLisenear{
        public void onSroll();
    }
...
//                cancel = (actiondownX < mScreenWidth / 2 && mx < mTouch.x) || (actiondownX > mScreenWidth / 2 && mx > mTouch.x);
                if ((Math.abs(mx - dx) > 10) && (Math.abs(my - dy) > 10)) {
                    isReturn = false;
//                    return false;
                }
                if ((t1 - et > 1000)) {
...
//                                abortAnimation();
                                pagefactory.onDraw(mNextPageCanvas,pagefactory.getCurrentPage());
                            } else {
                                return false;
                            }
                        } else if (actiondownX >= mScreenWidth / 2) {// 从右翻
                            BookStatus status = pagefactory.nextPage();
                            Log.i("wzh","pagefactory.getCurrentPage(): "+pagefactory.getCurrentPage());
                            if (status == BookStatus.NO_NEXT_PAGE) {
                                ToastUtils.showSingleToast("没有下一页啦");
                                return false;
                            } else if (status == BookStatus.LOAD_SUCCESS) {
...
//                }
    ...
    public void nextPage() {
        ...
        if (status == BookStatus.NO_NEXT_PAGE) {
            ...
            if (isPrepared) {
                pagefactory.onDraw(mCurrentPageCanvas,pagefactory.getCurrentPage());
                pagefactory.onDraw(mNextPageCanvas,pagefactory.getNextPage());
                postInvalidate();
            }
        } else {
            return;
    ...
    public void prePage() {
        ...
        if (status == BookStatus.NO_PRE_PAGE) {
            ...
            if (isPrepared) {
                pagefactory.onDraw(mCurrentPageCanvas,pagefactory.getCurrentPage());
                pagefactory.onDraw(mNextPageCanvas,pagefactory.getPrePage());
                postInvalidate();
            }
        } else {
            return;
